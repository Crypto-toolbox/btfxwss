...
class BtfxWss:
    ...
    def _handle_subscribed(self, *args,  chanId=None, channel=None, **kwargs):
        ...
        try:
            ...
            self.channel_labels[chanId] = (channel, kwargs['pair'])
        except KeyError:
...
class BtfxWssRaw:
    ...
    def start(self, path=None):
        ...
        self.tickers = open(path + 'tickers.csv', 'a', encoding='UTF-8')
        self.books = open(path + 'books.csv', 'a', encoding='UTF-8')
        self.raw_books = open(path + 'raw_books.csv', 'a', encoding='UTF-8')
        self.trades = open(path + 'trades.csv', 'a', encoding='UTF-8')
        self.candles = open(path + 'candles.csv', 'a', encoding='UTF-8')
